...
def load_application(demo_id=None):
    actionUrl = url_for('DynamiqueIvrIhm.saveApplication')
    laDemo = Demo.query.filter(Demo.id == demo_id).first()
    return render_template('formulare.html', action="Modifier", actionUrl=actionUrl, value=laDemo.value,
                           number=laDemo.number, id=laDemo.id)
